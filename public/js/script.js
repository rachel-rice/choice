...
// Create new list for guests (stored in localStorage)
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (createForm) {
    createForm.addEventListener('submit', (e) => {
      e.preventDefault(); // prevent normal POST for guests

      // Logged in → allow normal server POST
      if (window.APP_USER) {
        createForm.submit();
        return;
      }

      // Guest → handle locally
      const nameInput = document.getElementById('listName');
      const name = nameInput.value.trim();
      if (!name) return alert('Please enter a name for your list.');
      ...
      // Refresh page or re-render lists
      location.reload();
    });
  }
});
...
async function deleteList(id) {
  if (window.APP_USER) {
    ...
    try {
      const response = await fetch(`/lists/delete/${id}`, { method: 'DELETE' });
